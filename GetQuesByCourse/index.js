    ...
    try {
        ...
        for (const subject of subjectData) {
            ...
            for (const topic of topicData) {
                ...
                const questionsCollection = await mongoose.connection.collection(`questions_${collectionName}`);
                const questionData = await questionsCollection.find({ 'topicId': topic._id }).toArray();

                // Structure the topic data with questions
            ...
            // Fetch sub-topics for the subject
            const subTopicsCollection = mongoose.connection.collection('subtopics');
            const subTopicData = await subTopicsCollection.find({ 'subjectId': subject._id }).toArray();

            // Create an array to store the structured sub-topics data
            const structuredSubTopics = [];

            for (const subTopic of subTopicData) {
                // Fetch questions for the sub-topic
                const questionsCollection = mongoose.connection.collection('questions');
                const questionData = await questionsCollection.find({ 'subTopicId': subTopic._id }).toArray();

                // Structure the sub-topic data with questions
                const structuredSubTopic = {
                    _id: subTopic._id,
                    name: subTopic.name,
                    questions: questionData
                };

                structuredSubTopics.push(structuredSubTopic);
            }

            // Structure the subject data with topics and sub-topics
        ...
        // Add subjects, topics, subtopics, and questions inside the courseData
