...
exports.handler = async (event, context, callback) => {
    ...
    if (type.toLowerCase() == "questions") {
        ...
        const query = topicId ? { 'topicId': new ObjectId(topicId), disabled: false } : { 'subTopicId': new ObjectId(subTopic), disabled: false };

        const datacount = await questions.find(query).toArray();
        const data = await questions.find(query).sort().skip(page * limit).limit(limit).toArray();
